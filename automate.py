...
with center_column:
    ...
    if df is not None and target_column_name and selected_task:
        ...
        if X is not None and y is not None:
            ...
            chat_input = st.text_input("Ask me anything about your analysis (e.g., 'What is the best model?', 'How good is the performance?', 'What about the test size?')")
            if chat_input:
                chat_input = chat_input.lower()
                if "best model" in chat_input:
                    if overall_best_model:
                        st.write(f"Agent AI: The best model for this task is **{overall_best_model}** with a performance metric of **{overall_best_metric:.4f}** achieved with a **{optimal_test_size*100:.0f}%** test split.")
                    else:
                        st.write("Agent AI: I haven't been able to determine the best model yet. Please ensure data is loaded and processed correctly.")
                elif "model performance" in chat_input or "how good" in chat_input:
                    if overall_best_model:
                        st.write(f"Agent AI: The performance of the **{overall_best_model}** model is **{overall_best_metric:.4f}**.")
                        metric_type = 'R2 Score' if selected_task == 'Regression' else 'Accuracy'
                        st.write(f"This means the model achieved a {metric_type} of **{overall_best_metric:.4f}**.")
                        if selected_task == 'Regression':
                            st.write(f"An R2 score of {overall_best_metric*100:.2f}% indicates that this percentage of the variance in the target variable can be explained by the model.")
                        else:
                            st.write(f"An accuracy of {overall_best_metric*100:.2f}% means the model correctly classified this percentage of samples.")

                        if overall_best_metric >= 0.8:
                            st.write("Agent AI: This is a **very good** performance! Your model is highly accurate/predictive.")
                        elif overall_best_metric >= 0.6:
                            st.write("Agent AI: This is a **good** performance. There might be room for further improvement with more advanced tuning or feature engineering.")
                        else:
                            st.write("Agent AI: The performance is moderate. You might consider more data, different features, or deeper model tuning.")
                    else:
                        st.write("Agent AI: I need to analyze the data first to tell you about model performance.")
                elif "test size" in chat_input or "split" in chat_input:
                    if optimal_test_size:
                        st.write(f"Agent AI: The optimal test size found for your dataset is **{optimal_test_size*100:.0f}%**. This split provided the best balance for evaluating the models and achieving the highest performance.")
                    else:
                        st.write("Agent AI: I'm still determining the optimal test size. Please wait for the analysis to complete.")
                elif "hello" in chat_input or "hi" in chat_input:
                    st.write("Agent AI: Hello there! How can I assist you with your machine learning task today?")
                else:
                    st.write(f"Agent AI: I'm happy to chat with you about data science! You said: '{chat_input}'. Try asking about 'best model', 'model performance', or 'test size'.")
        else:
